...
| [Project](LIFECYCLE.md) | per project: several hours, one or several PRs | Templated initiation anchored on a GitHub issue, with the acceptance template (`ACCEPTANCE.md`) of every touched subproject; Architect handoff (PRD, ARD, plan, handoff); agent-team execution with acceptance criteria, worktree/PR, evidence, CI/CD; acceptance LGTM | Article with lessons and the axis-alignment check (Architect and Builder levels), triggering Atlas updates |

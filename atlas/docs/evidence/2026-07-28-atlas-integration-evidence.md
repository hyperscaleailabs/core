...
| Module articles with Lab Notes entries | 2 of 2 |
...
Search index written: 34 documents -> public/search-index.json
...
[build] 47 page(s) built in 629ms
[build] Complete!
...
| Entries resolve | `--all --check` after intake | **passed**, 2 of 2 |
| Drafting works from a real article | intake of the `models` and `prod` articles | entry written with title, description, provenance, tier, and a heading skeleton, marked `draft: true` |
...
|------------|----------------|
| [2026-07-28-atlas-home.png](2026-07-28-atlas-home.png) | The site builds and serves in the monorepo: hero, the article count reading **34**, and the section grid |
| [2026-07-28-atlas-lab-index.png](2026-07-28-atlas-lab-index.png) | The new `lab` section renders as a first-class section with both entries, their levels, reading times, and tags |
| [2026-07-28-atlas-lab-article.png](2026-07-28-atlas-lab-article.png) | A published lab note end to end: metadata, table of contents, provenance blockquote linking back to the module article, and the "Edit this page" link |

Outbound links were read from the rendered DOM rather than from the source, so
the assertion is about what a reader would click:
